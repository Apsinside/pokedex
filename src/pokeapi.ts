...
export class PokeAPI {
  ...
  constructor(cacheIntervall: number) {
    this.cache = new Cache(cacheIntervall);
  }

  async fetchLocations(pageURL?: string): Promise<ShallowLocations> {
    const url = pageURL || `${PokeAPI.baseURL}/location-area`;

    const cachedRequest = this.cache.get<ShallowLocations>(url);
    if(cachedRequest){
  ...
  async fetchLocation(locationName: string): Promise<Location> {
    ...
    const cachedRequest = this.cache.get<Location>(url);
    if(cachedRequest){
        return cachedRequest;
    }
    ...
    async fetchPokemon(pokemonName: string): Promise<Pokemon> {
        ...
        const cachedRequest = this.cache.get<Pokemon>(url);
        if(cachedRequest){
            return cachedRequest;
        }
